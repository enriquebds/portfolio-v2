...
function tokenize(code: string) {
  ...
  const patterns = [
    { type: 'comment', regex: /^(\/\/.*|\/\*[\s\S]*?\*\/)/ },
    {
      type: 'keyword',
      regex:
        /^(import|export|from|function|const|let|var|return|if|else|typeof|null|undefined|true|false|default|new|class|extends|async|await|of|in|for|while|do|try|catch|finally|throw|type|interface|enum)\b/,
    },
...
export function CodeSnippet({
    ...
    if (token.type === 'newline') {
      ...
      currentLine.push(
        <span
          ...
          style={{ color: TOKEN_COLORS[token.type] || '#D4D4D4' }}
        >
          {token.value}
        </span>,
      )
  ...
  const content = (
    <div
      className={cn('rounded-xl overflow-hidden shadow-2xl', className)}
      style={{
        background: '#1E1E1E',
    ...
    >
      ...
      <div className="p-4 overflow-x-auto">
        <div className="min-w-0">{elements}</div>
      </div>
    </div>
  )
