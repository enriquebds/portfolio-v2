...
async function getData() {
  try {
    ...
    const experiences: ExperienceItem[] = experienceRes.docs.map((doc: any) => ({
      id: String(doc.id), company: doc.company || '', role: doc.role || '',
      startDate: doc.startDate || '', endDate: doc.endDate || null,
      location: doc.location || '',
      bullets: (doc.description || []).map((d: any) => d.bullet || '').filter(Boolean),
      stack: (doc.stack || []).map((s: any) => s.tech || '').filter(Boolean),
      current: doc.current || false, order: doc.order || 0,
    }))
    const projects: ProjectItem[] = projectsRes.docs.map((doc: any) => ({
      id: String(doc.id), title: doc.title || '', slug: doc.slug || '',
      description: doc.description || '',
      stack: (doc.stack || []).map((s: any) => s.tech || '').filter(Boolean),
      githubUrl: doc.githubUrl || undefined, liveUrl: doc.liveUrl || undefined,
      status: doc.status || 'draft', featured: doc.featured || false, order: doc.order || 0,
    }))
    const certifications: CertificationItem[] = certsRes.docs.map((doc: any) => ({
      id: String(doc.id), title: doc.title || '', issuer: doc.issuer || '',
      type: doc.type || 'certification', date: doc.date || '',
      credentialUrl: doc.credentialUrl || undefined, order: doc.order || 0,
    }))
    const skills: SkillItem[] = skillsRes.docs.map((doc: any) => ({
      id: String(doc.id), name: doc.name || '', category: doc.category || 'frontend',
      level: doc.level || 'intermediario', icon: doc.icon || '', order: doc.order || 0,
    }))
    return { experiences, projects, certifications, skills }
  } catch {
    return {
      experiences: [] as ExperienceItem[], projects: [] as ProjectItem[],
      certifications: [] as CertificationItem[], skills: [] as SkillItem[],
    }
  }
}
